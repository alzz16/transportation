...
export default function IncidentWidget({ incidents }: IncidentWidgetProps) {
  ...
  return (
    <div className="w-full glass-panel p-5 flex flex-col gap-4 border-l-4 border-l-amber-400">
      ...
      <div className="flex flex-col gap-3.5">
        {incidents.map((inc) => {
          ...
          return (
            <div
              key={inc.id}
              className={`p-3.5 rounded-2xl border flex flex-col gap-2 ${borderClass} transition-all`}
            >
              <div className="flex items-center justify-between">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={`text-[9px] font-bold ${badgeClass} border px-2 py-0.5 rounded-full`}>
                    {inc.transitName}
                  </span>
                  <h3 className={`text-xs font-bold ${titleColor}`}>{inc.title}</h3>
                </div>
                <span className="text-[9px] text-slate-400 font-bold">{inc.updatedAt}</span>
              </div>
              ...
              {inc.sourceName && inc.sourceUrl && (
                <div className="pl-4 mt-1 flex items-center gap-1">
                  ...
                  <a
                    ...
                    className="text-[9px] font-bold text-accent-primary hover:text-indigo-500 hover:underline inline-flex items-center gap-0.5"
                  >
                    {inc.sourceName}
